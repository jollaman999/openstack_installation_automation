...
resource "null_resource" "install_kolla_ansible_install_needed_python_packages" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "fi",
            "echo \"[*] Installing pbr...\"",
            "pip3 install --break-system-packages -U pbr",
            "STATUS=`echo $?`",
            "if [ $STATUS != 0 ]; then",
...
resource "null_resource" "install_kolla_ansible_configure_kolla_ansible_global_variables" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            "sed -i 's/^#enable_manila_backend_generic:.*/enable_manila_backend_generic: \"yes\"/g' /etc/kolla/globals.d/globals.yml",
            "sed -i '/podman_registry_insecure/d' /etc/kolla/globals.d/globals.yml",
            "echo 'podman_registry_insecure: \"yes\"' >> /etc/kolla/globals.d/globals.yml",
...
resource "null_resource" "post_install_setup_loadbalancer_interface" {
    ...
    provisioner "remote-exec" {
        inline = [
            ...
            ". /etc/kolla/admin-openrc.sh",
            "pip3 install --break-system-packages python-openstackclient python-glanceclient python-neutronclient",
